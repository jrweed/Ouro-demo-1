...
import { AIPricingSidebar, MOCK_PRICING_DATA } from "./AIPricingSidebar";
...
export function InputLoadForm() {
  ...
  const fetchPricing = useCallback((origin: LocationResult | null, dest: LocationResult | null) => {
    if (!origin || !dest) {
      setPricingData(null);
      return;
    }
  ...
  useEffect(() => {
    fetchPricing(confirmedOrigin, confirmedDest);
  }, [confirmedOrigin, confirmedDest, fetchPricing]);
  ...
  const sectionComplete = [
    routeReady,
    !!values.pickupDate,
    !!values.equipmentType && !!values.commodity && !!values.temperature,
    true,
  ];
  ...
  function validate(): boolean {
    ...
    if (!values.temperature) next.temperature = "Required";
    setErrors(next);
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. Strawberries, Frozen Shrimp"
                    className={inputClass}
                  />
                  {errors.commodity && (
