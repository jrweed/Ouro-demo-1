/**
 * ColdHaul Matching Algorithm — TypeScript implementation
 *
...
function applyHardFilters(
  ...
  for (const c of carriers) {
    ...
    for (const t of carrierTrucks) {
      if (t.current_load_status === "on_load") continue;
      if (load.required_equipment_types.length > 0 && !load.required_equipment_types.includes(t.equipment_type)) continue;
      if (t.temp_capability_min > load.required_temp_min) continue;
      if (t.temp_capability_max < load.required_temp_max) continue;
      if (t.max_payload_lbs < load.weight_lbs) continue;
...
export function loadToMatchingFormat(storedLoad: {
  ...
  const weightNum = typeof storedLoad.weightLbs === "number"
    ? storedLoad.weightLbs
    : parseInt(storedLoad.weightLbs || "40000", 10);

  return {
    id: storedLoad.id,
    broker_id: "b1000000-0000-0000-0000-000000000001", // Default to Broker 1 for demo
    commodity_type: commodityMap[storedLoad.commodity || ""] || "produce",
    required_temp_min: tempNum - 3,
    required_temp_max: tempNum + 6,
    required_equipment_types: eqMap[storedLoad.equipmentType || "reefer_single"] || ["reefer_53"],
