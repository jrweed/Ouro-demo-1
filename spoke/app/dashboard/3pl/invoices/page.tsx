...
function InvoiceRow({ invoice }: { invoice: Invoice }) {
  ...
  return (
    <div className="grid items-center border-b border-[#f3f4f6] px-5 py-4 last:border-none hover:bg-[#fafafa] transition-colors"
    ...
    >
      ...
      <div className="min-w-0 pr-4">
        ...
        <p className="mt-0.5 text-[12px] text-[#9ca3af]">{invoice.carrierName}</p>
      </div>
...
export default function ThreePLInvoicesPage() {
  ...
  // Ensure invoices exist for all bookings
  useEffect(() => {
    if (!user) return;
        ...
        <div className="mb-7 grid grid-cols-3 gap-4">
          <div className="rounded-xl border border-[#e5e7eb] bg-white p-5">
            ...
            <p className="mt-2 text-[26px] font-bold tracking-tight text-[#111827]">
              ${(pendingTotal + paidTotal).toLocaleString()}
            </p>
          ...
          <div className="rounded-xl border border-[#fffbeb] bg-[#fffbeb]/50 p-5">
            <p className="text-[11px] font-semibold uppercase tracking-wider text-[#d97706]">
              Pending
            </p>
            <p className="mt-2 text-[26px] font-bold tracking-tight text-[#111827]">
              ${pendingTotal.toLocaleString()}
            </p>
            <p className="mt-0.5 text-[12px] text-[#9ca3af]">{counts.pending} outstanding</p>
          </div>
          <div className="rounded-xl border border-[#f0fdf4] bg-[#f0fdf4]/50 p-5">
            ...
            <p className="mt-0.5 text-[12px] text-[#9ca3af]">{counts.paid} collected</p>
          </div>
        </div>
      )}
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-[#e5e7eb] bg-white py-20 text-center">
          ...
          <p className="mt-1 text-xs text-[#9ca3af]">
            Invoices are automatically created when a booking is confirmed.
          </p>
          <Link
          ...
          >
            <DollarSign size={15} /> View Bookings
          </Link>
        </div>
      ) : (
