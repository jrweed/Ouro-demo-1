...
import { EQUIPMENT_TYPES, STATUS_CONFIG, isReeferEquipment } from "@/lib/utils/constants";
...
const BLANK_ADD_FORM = {
  ...
  make: "",
  model: "",
  equipmentType: "dry_van" as TruckType["equipmentType"],
  tempMin: "",
  tempMax: "",
};
    ...
    <>
      ...
      <div className="fixed right-0 top-0 z-40 flex h-full w-[400px] flex-col border-l border-[#e5e7eb] bg-white shadow-xl">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          {/* Temperature range — only for reefer equipment */}
          {isReeferEquipment(form.equipmentType) && (
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="mb-1.5 block text-[12px] font-semibold uppercase tracking-wider text-[#6b7280]">Temp Min (°F)</label>
              <input
                type="number"
                placeholder="-20"
                value={form.tempMin}
                onChange={(e) => field("tempMin", e.target.value)}
                className="w-full rounded-lg border border-[#d1d5db] px-3 py-2.5 text-sm text-[#111827] outline-none focus:border-[#3b82f6] focus:ring-2 focus:ring-[#3b82f6]/10"
              />
            </div>
            <div>
              <label className="mb-1.5 block text-[12px] font-semibold uppercase tracking-wider text-[#6b7280]">Temp Max (°F)</label>
              <input
                type="number"
                placeholder="70"
                value={form.tempMax}
                onChange={(e) => field("tempMax", e.target.value)}
                className="w-full rounded-lg border border-[#d1d5db] px-3 py-2.5 text-sm text-[#111827] outline-none focus:border-[#3b82f6] focus:ring-2 focus:ring-[#3b82f6]/10"
              />
            </div>
          </div>
