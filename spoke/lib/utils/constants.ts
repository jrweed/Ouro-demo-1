...
export const REEFER_EQUIPMENT_TYPES = ["reefer_single", "reefer_multi"];

export const isReeferEquipment = (type: string) =>
  REEFER_EQUIPMENT_TYPES.includes(type);

export function equipmentLabel(val?: string): string {
  if (!val) return "—";
  const found = EQUIPMENT_TYPES.find((t) => t.value === val);
  if (found) return found.label;
  // Handle matching-service internal types
  if (val === "reefer_53") return "Reefer 53'";
  if (val === "reefer_48") return "Reefer 48'";
  if (val === "power_only") return "Power Only";
  return val;
}
